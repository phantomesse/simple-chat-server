...
public class Server {
	...
	public Server(int portNumber) {
		...
		try {
			...
			try {
				while (true) {
					new ServerThread(this, listener.accept()).start();
				}
			} finally {
				listener.close();
			}
	...
	public String processClientInput(String clientInput) {
		...
		if (clientInput.equals("whoelse")) {
			...
			// TODO
		} else if (clientInput.startsWith("hello") || clientInput.startsWith("hi") || clientInput.startsWith("hey")) {
			// Responds with a random greeting
			String[] greetings = { "Hi!", "Hello.", "Greetings.", "Howdy!", "Hey!", "Yo." };
